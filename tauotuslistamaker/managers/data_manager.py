...
class DataManager:
    ...
    def __is_valid_time_format(self, time_str: str) -> bool:
        try:
    ...
    def __check_required_keys(self, dictionary: dict, required_keys: list) -> None:
        for key in required_keys:
            if key not in dictionary:
                raise KeyError(f"Required keys for dictionary: {required_keys}")
    
    def __validate_cashiers_data(self):
        for cashier in self.cashiers:
            ...
            if not self.__is_valid_time_format(cashier["shift_start"]) or not self.__is_valid_time_format(cashier["shift_end"]):
                raise ValueError("Shift start and end must be in 'HH:MM' format.")
        
    def __transform_cashiers_shift_intervals_to_TimeInterval_objects(self):
        for cashier in self.cashiers:
    ...
    def load_config(self, file_name: str) -> None:
        try:
            with open(self.ROOT_FOLDER / file_name, "r") as json_file:
                ...
                self.__transform_config_time_intervals_to_TimeInterval_objects()
        except FileNotFoundError:
            raise FileNotFoundError(f"{file_name} not found")
        except json.decoder.JSONDecodeError:
            raise ValueError(f"{file_name} is not a valid json file")
    ...
    def __validate_config_data(self):
        ...
        self.__check_required_keys(self.config, config_required_keys)
        ...
        for key in keys_for_lists_with_whole_numbers:
            for element in self.config[key]:
                if not self.__is_whole_number(element):
        ...
        self.__check_required_keys(self.config["opening_hours"], opening_hours_required_keys)
        if not self.__is_valid_time_format(self.config["opening_hours"]["opening_time"]) or not self.__is_valid_time_format(self.config["opening_hours"]["closing_time"]):
        ...
        for self_service_checkout in self.config["self_service_checkouts"]:
            self.__check_required_keys(self_service_checkout, self_service_checkouts_required_keys)
            for key in self_service_checkout:
                ...
                match key:
                    ...
                    case "opening_time" | "closing_time":
                        if not self.__is_valid_time_format(self_service_checkout_value):
    ...
    def __transform_config_time_intervals_to_TimeInterval_objects(self):
    ...
    def __is_whole_number(self, number) -> bool:
